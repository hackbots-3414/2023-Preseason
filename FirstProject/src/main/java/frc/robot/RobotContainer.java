...
public class RobotContainer {
  ...
  private CommandSelector select() {
    double choice_number = SmartDashboard.getNumber("autocommand", 1);
    ...
    } else if (choice_number == 3.14) {
      return CommandSelector.PI;
    } else {
  ...
  private final Command m_exampleSelectCommand =
      new SelectCommand(
          ...
          Map.ofEntries(
              ...
              Map.entry(CommandSelector.FOUR, new PrintCommand("Command four was selected!")),
              Map.entry(CommandSelector.PI, new PrintCommand("That's pi! Want a slice?"))),
          this::select);

  public RobotContainer() {
    // Configure the button bindings
    configureButtonBindings();
    SmartDashboard.putNumber("autocommand", 3);
  }
